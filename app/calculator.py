...
# Handler that opens the file for each emit and closes it immediately.
# This avoids holding a persistent file handle which on Windows would block
# deleting the file's parent temporary directory during tests.
class TransientFileHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        try:
            ...
            # Open, write, and close per-record to avoid keeping the file open.
            with open(self.filename, 'a', encoding=self.encoding or 'utf-8') as f:
...
class Calculator:
    ...
    def _setup_logging(self) -> None:
        ...
        try:
            ...
            # Create a transient file handler that opens/closes on each emit so
            # the file will not remain locked by the process (helps Windows tests).
            file_handler = TransientFileHandler(filename=str(log_file), encoding=self.config.default_encoding)
            formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')
            file_handler.setFormatter(formatter)
            file_handler.setLevel(logging.INFO)

            # Attach the handler to the root logger so module-level logging calls
            # (e.g., logging.info) are captured and written to the file.
            root_logger = logging.getLogger()
            root_logger.addHandler(file_handler)
            self._file_handler = file_handler

            # Log via the root logger so messages are written to the file handler
    ...
    def close_logging(self) -> None:
        """
        Remove and close the file handler attached to the root logger.

        Ensures file descriptors are released so temporary directories can be removed.
        """
        try:
